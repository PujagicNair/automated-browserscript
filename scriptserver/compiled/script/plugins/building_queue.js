            ...
            return new Promise(async (resolve) => {
                if (hack.screen == 'main') {
                    ...
                    if (!hasQueue) {
                        ...
                        let rows = await hack.browser.selectMultiple('#buildqueue > tr', 'className');
                        let buildRows = rows.filter(row => ~row.indexOf('buildorder_')).map(row => rows.indexOf(row) + 1);
                        let queue = [];
                        for (let rowIndex of buildRows) {
                            let row = {};
                            let rowItems = await hack.browser.selectMultiple(`#buildqueue > tr:nth-of-type(${rowIndex}) > td.lit-item`, 'innerText');
