    ...
    pageControl: {
        ...
        server: function (browser, input, output) {
            ...
            async function sendPage() {
                let html = await browser.page.evaluate(() => document.all[0].innerHTML);
        ...
        client: function (window, input, output) {
            let uniqueSelector = e => { let l; for (; e;) {
                let t = e.localName;
                if (!t)
                    break;
                if (e.id)
                ...
                if (n) {
                    ...
                    if (l.length > 1) {
                        let n = 0;
                        const a = [...l].findIndex(l => (e.localName === l.localName && n++, l === e)) + 1;
                        a > 1 && n > 1 && (t += ":nth-child(" + a + ")");
                    }
                }
                l = t + (l ? ">" + l : ""), e = n;
            } return l; };
            ...
            input(data => {
                if (data.type == "html") {
                    ...
                    window.document.querySelectorAll('input[type=text], textarea').forEach((input) => {
